...
{8.12,1.49,2.71,4.32,12.02,2.3,2.03,5.92,7.31,0.1,0.69,3.98,2.61,6.95,7.68,1.82,0.11,6.02,6.28,9.1,2.88,1.11,2.09,0.17,2.11,0.07};
	 ...
	 * @throws Exception 
	 **/
	public static double getIC(byte[] ar, String fileName) throws Exception
	{
		int ic = 0;
		double answer=0;
	...
	public static double getIC(byte[] ar, int trials) throws Exception
	{
		int ic = 0;
		double answer=0;
		//byte[] pt = CryptoTools.fileToBytes(fileName);
		//byte[] pt = CryptoTools.clean(raw);
		//int trials = 1500;
		for (int i = 1; i <= trials; i++)
		{
	...
	/**
	 * Writes a character after every skip to the output array. 
	 * @param ar
	 * @param skip
	 * @param start
	 * @return
	...
	public static byte[] sample(byte[] ar, int skip, int start)
	{
		...
		//int pos = 0;
		for (int i = start; i < ar.length; i = i + skip)
		{
			baos.write(ar[i]);
			//result[pos] = ar[i];
			//pos++;
